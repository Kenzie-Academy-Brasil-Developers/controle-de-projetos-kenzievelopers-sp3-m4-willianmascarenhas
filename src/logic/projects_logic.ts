...
const createProjectsTechnologies = async (req: Request, res: Response): Promise<Response> => {
    ...
    const queryResult2: QueryResult = await client.query(querConfig2)
...
const dellTecFromProject = async(req: Request, res: Response): Promise<Response> => {
    const projectId:number = parseInt(req.params.id)
    ...
    const getIdTecResult: QueryResult<ITecnologie> = await client.query(getIdTecConfig)
...
export { createProject, getProject, patchProject, deleteProject, createProjectsTechnologies, dellTecFromProject} 