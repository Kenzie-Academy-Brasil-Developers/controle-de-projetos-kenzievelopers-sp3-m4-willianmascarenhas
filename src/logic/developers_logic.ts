import { IDevInfo, IDeveloper_infos, IDevelopers, TDeveloper_infosRequest, TDevelopersRequest } from "../interfaces/developers_interface";
...
const createDeveloper = async (req: Request, res: Response): Promise<Response> =>{
    ...
    const queryResults: QueryResult<IDevelopers> = await client.query(queryString)
...
const createDeveloperInfo = async (req: Request, res: Response): Promise<Response> => {
    ...
    const queryResult: QueryResult<IDeveloper_infos> = await client.query(queryString)
    
    return res.status(201).json(queryResult.rows[0])
}

const getDeveloperById = async (req: Request, res: Response): Promise<Response> => {
...
const patchDeveloper = async (req: Request, res: Response): Promise<Response> => {
    const id:number = parseInt(req.params.id)
...
export { createDeveloper, createDeveloperInfo, getDeveloperById, patchDeveloper, deleteDeveloper }
